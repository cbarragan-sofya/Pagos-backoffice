 ...
 */
package com.sofya.java.uphone.apiRest.paymentPointBackOffice.jwt;
...
public class JWebToken {
    ...
    private static String SECRET_KEY; //@TODO Add Signature here
    ...
    private JWebToken() {
        ...
        SECRET_KEY = ConfigManager.get("SECRET_KEY");
        HEX_ARRAY = ConfigManager.get("HEX_ARRAY").toCharArray();
        ISSUER = ConfigManager.get("ISSUER");
    ...
    public boolean isValid() {
        ...
        try {
            flag = payload.getLong("exp") > (LocalDateTime.now().toEpochSecond(ZoneOffset.UTC)) //token not expired
    ...
    public boolean isExpired() {
        Boolean flag = Boolean.FALSE;
        try {
            flag = payload.getLong("exp") < (LocalDateTime.now().toEpochSecond(ZoneOffset.UTC));
        } catch (Exception e) {
        }
        return flag;
    }
